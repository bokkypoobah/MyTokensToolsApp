const Wallets = {
  template: `
    <div>
      ...
      <b-button v-b-toggle.contracts size="sm" block variant="outline-info">Wallets</b-button>
  ...
  methods: {
    async timeoutCallback() {
      // logDebug("Wallets", "timeoutCallback() count: " + this.count);
      ...
      logDebug("Wallets", "before tokens/loadLibrary");
      await store.dispatch('tokens/loadLibrary');
      logDebug("Wallets", "after tokens/loadLibrary");
  ...
  // beforeDestroy() {
  //   logDebug("Wallets", "beforeDestroy()");
  // },
  mounted() {
    logDebug("Wallets", "mounted()");
...
const walletsModule = {
  ...
  mutations: {
    ...
    updateAssetsCompletion(state) {
      ...
      // for (const contract in state.collectionList) {
      //   const collection = state.collections[state.collectionList[contract]];
      //   logDebug("walletsModule", "mutations.updateAssetsCompletion() - collection: " + JSON.stringify(collection, null, 2));
      ...
      //     logDebug("walletsModule", "mutations.updateAssetsCompletion()   - asset: " + JSON.stringify(asset));
      //   }
      // }
    },
    updateAssets(state, { owner, permissions, data }) {
      // logDebug("walletsModule", "mutations.updateAssets(" + JSON.stringify(permissions) + ", " + JSON.stringify(data).substring(0, 100) + ")");
    ...
    updateNFTData(state, nftData) {
      // logDebug("walletsModule", "mutations.updateNFTData(" + JSON.stringify(nftData) + ")");
    ...
    updateSelectedId(state, selectedId) {
      ...
      logDebug("walletsModule", "updateSelectedId('" + JSON.stringify(selectedId) + "')")
    },
    updateBalances(state, balances) {
      state.balances = balances;
      logDebug("walletsModule", "updateBalances('" + JSON.stringify(balances) + "')")
    },
    updateParams(state, params) {
      state.params = params;
      logDebug("walletsModule", "updateParams('" + params + "')")
    },
    updateExecuting(state, executing) {
      state.executing = executing;
      logDebug("walletsModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
    async loadLibrary(context) {
      logDebug("walletsModule", "actions.loadLibrary()");
      ...
      // logDebug("walletsModule", "actions.loadLibrary() - defaultRegistryEntries[0].length: " + defaultRegistryEntries[0].length);
      ...
      for (i = 0; i < defaultRegistryEntries[0].length; i++) {
        ...
        // logDebug("walletsModule", "  owner: " + owner + ", contract: " + contract + ", permission: " + permission + ", curation: " + curation);
      }
      // logDebug("walletsModule", "actions.loadLibrary() - owners: " + JSON.stringify(owners));
      logDebug("walletsModule", "actions.loadLibrary() - permissions: " + JSON.stringify(permissions));
      ...
      for (const [owner, ownersContracts] of Object.entries(owners)) {
        ...
        if (hasNull) {
          ...
          while (!completed) {
            ...
            // logDebug("walletsModule", "actions.loadLibrary() owner url:" + url);
            ...
            //     // logDebug("walletsModule", "actions.loadLibrary() - asset(" + (parseInt(offset) + assetIndex) + ") name: " + asset.collection.name + ", slug: " + asset.collection.slug + ", owner: " + asset.owner.address + ", contract: " + asset.asset_contract.address + ", permission: " + JSON.stringify(permission));
          ...
          for (let i = 0; i < ownersContracts.length; i++) {
            ...
            while (!completed) {
              ...
              logDebug("walletsModule", "actions.loadLibrary() owner and contract url:" + url);
              const data = await fetch(url).then(response => response.json());
              context.commit('updateAssets', { owner, permissions, data });
              // if (data.assets && data.assets.length > 0) {
              //   for (let assetIndex = 0; assetIndex < data.assets.length; assetIndex++) {
        ...
        //   logDebug("walletsModule", "actions.loadLibrary():" + key + " => " + JSON.stringify(asset).substring(0, 100));
        // }
      }
    },
    updateNFTData(context, nftData) {
      // logDebug("walletsModule", "actions.updateNFTData(" + JSON.stringify(nftData) + ")");
      context.commit('updateNFTData', nftData);
    },
    updateSelectedId(context, selectedId) {
      logDebug("walletsModule", "actions.updateSelectedId(" + JSON.stringify(selectedId) + ")");
    ...
    async execWeb3({ state, commit, rootState }, { count }) {
      ...
      logDebug("walletsModule", "execWeb3() start[" + count + "] rootState.route.params: " + JSON.stringify(rootState.route.params) + ", networkUpdated: " + networkUpdated + ", blockUpdated: " + blockUpdated + ", coinbaseUpdated: " + coinbaseUpdated+ ", powerOn: " + powerOn + ", connected: " + connected + ", coinbase: " + coinbase);
      if (coinbase != null) {
        if (!state.executing) {
          commit('updateExecuting', true);
          logDebug("walletsModule", "execWeb3() executing[" + count + ", " + JSON.stringify(rootState.route.params) + ", " + networkUpdated + ", " + blockUpdated + ", " + coinbaseUpdated + "]");

          var paramsChanged = false;
          if (state.params != rootState.route.params.param) {
            logDebug("walletsModule", "execWeb3() params changed from " + state.params + " to " + JSON.stringify(rootState.route.params.param));
            paramsChanged = true;
          ...
          if (networkUpdated || blockUpdated || coinbaseUpdated || paramsChanged) {
            ...
            logDebug("walletsModule", "execWeb3() nftAddress: " + nftAddress);

            const nftAbi = ERC1155NFTABI;
            // logDebug("walletsModule", "execWeb3() nftAbi: " + JSON.stringify(nftAbi));
            ...
            logDebug("walletsModule", "execWeb3() coinbase: " + coinbase);
            ...
            logDebug("walletsModule", "execWeb3() moonCatRescue.totalSupply: " + totalSupply.toString());
            const rescueIndex = await moonCatRescue.rescueIndex();
            logDebug("walletsModule", "execWeb3() moonCatRescue.rescueIndex: " + rescueIndex.toString());
            // const catIds = await moonCatRescue.getCatIds();
            // logDebug("walletsModule", "execWeb3() moonCatRescue.catIds: " + JSON.stringify(catIds));
            ...
            logDebug("walletsModule", "execWeb3() acclimatedMoonCatRescue.totalSupply: " + totalSupplyAcclimated.toString());
            ...
            logDebug("walletsModule", "execWeb3() wrappedMoonCatRescue.totalSupply: " + totalSupplyWrapped.toString());
            const _tokenIDToCatID = await wrappedMoonCatRescue._tokenIDToCatID(0);
            logDebug("walletsModule", "execWeb3() wrappedMoonCatRescue._tokenIDToCatID: " + _tokenIDToCatID.toString());

            if (false) {
            // console.table(provider);
            const name = await provider.lookupAddress(coinbase);
            logDebug("walletsModule", "execWeb3() coinbase: " + JSON.stringify(store.getters['connection/coinbase']) + " => " + name);
            ...
            logDebug("walletsModule", "execWeb3() beeeefRegistryContract.entries: " + JSON.stringify(entries));
            ...
            logDebug("walletsModule", "execWeb3() allnames: " + JSON.stringify(addresses) + " => " + allnames);
            const validNames = allnames.filter((n) => normalize(n) === n );
            logDebug("walletsModule", "execWeb3() validNames: " + JSON.stringify(addresses) + " => " + validNames);
            }
            ...
            // logDebug("walletsModule", "execWeb3() nftContract: " + JSON.stringify(nftContract));
            ...
            logDebug("walletsModule", "execWeb3() tokens/allTokenIds: " + JSON.stringify(store.getters['tokens/allTokenIds']));

            const balanceOfs = await nftContract.balanceOfBatch(accounts, tokenIds);
            logDebug("walletsModule", "execWeb3() balanceOfs: " + JSON.stringify(balanceOfs.map((x) => { return x.toString(); })));
            ...
            logDebug("walletsModule", "execWeb3() cpBalanceOf: " + cpBalanceOf);
            }
            ...
            //   if (i % 100 == 0) {
            //     logDebug("walletsModule", "execWeb3() owner: " + owner + " " + i);
            //   }
            ...
            req.onload  = function() {
              logDebug("walletsModule", "execWeb3() punkData txt: " + req.readyState + " => " + req.responseText);
              const punkData = JSON.parse(req.responseText);
              logDebug("walletsModule", "execWeb3() punkData: " + JSON.stringify(punkData));
            };
            req.send(null);
            ...
            req.open('GET', url, true);
            req.onload  = function() {
              logDebug("walletsModule", "execWeb3() openSeaPunkData txt: " + req.readyState + " => " + req.responseText);
              if (req.readyState == 4) {
                const openSeaPunkData = JSON.parse(req.responseText);
                logDebug("walletsModule", "execWeb3() openSeaPunkData JSON: " + JSON.stringify(openSeaPunkData));
              }
            };
            req.send(null);
            ...
            req.open('GET', url, true);
            req.onload  = function() {
              logDebug("walletsModule", "execWeb3() pixelPortraitsData txt: " + req.readyState + " => " + req.responseText);
              if (req.readyState == 4) {
                const pixelPortraitsData = JSON.parse(req.responseText);
                logDebug("walletsModule", "execWeb3() pixelPortraitsData JSON: " + JSON.stringify(pixelPortraitsData));
              }
            };
            req.send(null);
            ...
            logDebug("walletsModule", "execWeb3() pbBalanceOf: " + pbBalanceOf);
            ...
            for (let i = 0; i < pbBalanceOf; i++) {
                ...
                logDebug("walletsModule", "execWeb3() i: " + i + ", tokenId: " + tokenId);
            }
            */
            ...
            //
            // // logDebug("walletsModule", "execWeb3() state.tokenData: " + JSON.stringify(state.tokenData));
            // if (count == 1) {
            ...
            //     var _tokensInfo = promisify(cb => tokenToolz.getWalletsInfo(chunk, store.getters['connection/coinbase'], store.getters['optinoFactory/address'], cb));
            ...
            //     }
            //   }
            //   // logDebug("walletsModule", "timeoutCallback() - refreshed " + addressesLength);
            // }
          }
          commit('updateExecuting', false);
          logDebug("walletsModule", "execWeb3() end[" + count + ", " + networkUpdated + ", " + blockUpdated + ", " + coinbaseUpdated + "]");
        } else {
          logDebug("walletsModule", "execWeb3() already executing[" + count + ", " + networkUpdated + ", " + blockUpdated + ", " + coinbaseUpdated + "]");
        }
      }
    },
  },
  // mounted() {
  //   logDebug("walletsModule", "mounted() $route: " + JSON.stringify(this.$route.params));
  // },
};
